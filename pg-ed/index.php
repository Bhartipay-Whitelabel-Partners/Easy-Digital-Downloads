<?php
/*
Plugin Name: EDD - pg Gateway
Plugin URL: http://easydigitaldownloads.com/extension/pg-gateway
Description: A pg gateway for Easy Digital Downloads
Version: 1.0
Author: Rohit Kr Singh
Author URI: https://www.pg.com/
...
function pw_edd_register_gateway($gateways) {
	$gateways['pg_gateway'] = array('admin_label' => 'pg Gateway', 'checkout_label' => __('pg Gateway', 'pw_edd'));
	return $gateways;	
}
add_filter('edd_payment_gateways', 'pw_edd_register_gateway');

function pw_edd_pg_gateway_cc_form() {
	// register the action to remove default CC form
	return;
}
add_action('edd_pg_gateway_cc_form', 'pw_edd_pg_gateway_cc_form');
...
add_action('edd_gateway_pg_gateway', 'pw_edd_process_payment');
...
//Return Response
function edd_listen_for_pg_gateway() {

	if ( isset( $_POST['RESPONSE_CODE'] ) && $_POST['CUST_EMAIL'] != "" ) {
            do_action( 'edd_verify_pg_gateway' );    
                   
	}
}
add_action( 'init', 'edd_listen_for_pg_gateway' );



function edd_process_pg_gateway() {
    global $edd_options;
    $payment = edd_insert_payment($payment);
    $order_id=$payment;
          
		  if($_POST['STATUS']!='Captured'){
		            edd_send_back_to_checkout( '?payment-mode=pg_gateway' );            
		        }
           
           if($_POST['STATUS']=='Captured')
					{
						$payment_meta   = edd_get_payment_meta( $order_id );
						edd_insert_payment_note( $order_id, sprintf( __( 'Thank you for your order . Your transaction has been successful. pg Transaction ID: %s', 'edd' ) , $_POST['STATUS'] ) );
						...
						edd_record_gateway_error( __( 'pg Error', 'edd' ), sprintf( __( 'It seems some issue in server to server communication. Kindly connect with administrator.', 'edd' ), '' ), $payment_id );
						...
						wp_redirect( '?page_id=6&payment-mode=pg_gateway' );
					}
...
add_action( 'edd_verify_pg_gateway', 'edd_process_pg_gateway' );
...
function pw_edd_add_settings($settings) {
	...
	$pg_gateway_settings = array(
		array(
			'id' => 'pg_gateway_settings',
			'name' => '<strong>' . __('pg Gateway Settings', 'pw_edd') . '</strong>',
		...
		array(
			...
			'desc' => __('Transaction URL Provided by Pg', 'pw_edd'),
			'type' => 'text',
			'size' => 'regular'
		),
		array(
	...
	return array_merge($settings, $pg_gateway_settings);	
}
